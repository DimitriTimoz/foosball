...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load tournaments." }, { status: 400 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Unable to update the tournament." }, { status: 400 });
  }
}
